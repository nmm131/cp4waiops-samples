...
if [[ $SKIP_CONFIRM != "true" ]]; then
  ...
  if [[ ! -z "$AIOPS_NAMESPACED" ]]; then
      if [ $AIOPS_NAMESPACED == "true" ] && [ $DELETE_IAF == "false" ] && [ $DELETE_AIOPS_PROJECT == "true" ]; then
        log $ERROR "Cannot delete the AIOps project while leaving IAF components in a namespace scoped install. Please review the properties file. Exiting."
        exit 1
      fi
  fi
else
...
if [[ ! -z "$AIOPS_PROJECT"  ]]; then 
   ...
   # Check if namespace scoped install. If namespaced, pass in AIOPS_PROJECT, else OPERATORS_NAMESPACE
   if [[ $AIOPS_NAMESPACED == "true" ]] ; then
    unsubscribe "ibm-aiops-orchestrator" $AIOPS_PROJECT ""
   else
   ...
   echo "----Congratulations! Cloud Pak for Watson AIOps has been uninstalled!----"
