...
is_aiops_namespaced() {
    ...
    operator_name=$(oc get subscription.operators.coreos.com/ibm-aiops-orchestrator -n $OPERATORS_NAMESPACE -o name --ignore-not-found)
    if [[ ! -z "$operator_name" ]]; then
        # Operator is installed in all-ns mode
        AIOPS_NAMESPACED="false"
        echo "Uninstalling AIOps from the cluster scope."
    else
        # Check that the operator is in the ns the user provided
        operator_name=$(oc get subscription.operators.coreos.com/ibm-aiops-orchestrator -n $AIOPS_PROJECT -o name --ignore-not-found)

        if [[ ! -z "$operator_name" ]]; then
            # Operator is in the user provided ns, this is a namespaced install
            AIOPS_NAMESPACED="true"
            echo "Uninstalling AIOps from the namespace $AIOPS_PROJECT."

        else
            log $WARNING "There is no subscription for AIOps in the namespace $AIOPS_PROJECT or $OPERATORS_NAMESPACE, please review uninstall-cp4waiops-props.sh"
        fi
    fi
...
set_iaf_namespace() {
    # separate from aiops check for idempotency
    # TODO clean up and consolidate to single function
    operator_name=$(oc get subscription.operators.coreos.com -n $OPERATORS_NAMESPACE -l "operators.coreos.com/ibm-automation.$OPERATORS_NAMESPACE" --ignore-not-found)
    if [[ ! -z "$operator_name" ]]; then
        # IAF is installed in OPERATORS_NAMESPACE
        IAF_NAMESPACE=$OPERATORS_NAMESPACE
    else
        # Check that the operator is in the ns the user provided
        operator_name=$(oc get subscription.operators.coreos.com -n $AIOPS_PROJECT -l "operators.coreos.com/ibm-automation.$AIOPS_PROJECT" --ignore-not-found)

        if [[ ! -z "$operator_name" ]]; then
            # Operator is in the user provided ns, this is a namespaced install
            IAF_NAMESPACE=$AIOPS_PROJECT

        else
            log $WARNING "There is no subscription for IAF in the namespace $AIOPS_PROJECT or $OPERATORS_NAMESPACE"
        fi
    fi
...
unsubscribe () {
    ...
    if [[ ( -z "$operator_name" ) && ( ! -z  "$operator_label" ) ]]; then
       ...
       if [[ ! -z "$operator_name" ]]; then
        ...
        echo "---operator name is $operator_name dest_namespace is $dest_namespace "
        operator_exists=$(check_oc_resource_exists "subscription.operators.coreos.com" $operator_name $dest_namespace)
        echo "---does the operator exist? $operator_exists"
    else
...
delete_installation_instance () {
    ...
    if  [ `oc get installations.orchestrator.aiops.ibm.com $installation_name -n $project --ignore-not-found | wc -l` -gt 0 ] ; then
        ...
        log $INFO "Deleting ConnectorConfiguration resource..."
        connectorconfig_name=$(oc get connectorconfiguration -n $project -o name)
        oc delete connectorconfiguration $connectorconfig_name -n $project --ignore-not-found;
...
delete_iaf_bedrock () {
    ...
    echo "-----Unsubscribing IAF stuff in $IAF_NAMESPACE"
    unsubscribe "" $IAF_NAMESPACE "operators.coreos.com/ibm-automation.$IAF_NAMESPACE"
    unsubscribe "" $IAF_NAMESPACE "operators.coreos.com/ibm-automation-ai.$IAF_NAMESPACE"
    unsubscribe "" $IAF_NAMESPACE "operators.coreos.com/ibm-automation-core.$IAF_NAMESPACE"
    unsubscribe "" $IAF_NAMESPACE "operators.coreos.com/ibm-automation-elastic.$IAF_NAMESPACE"
    unsubscribe "" $IAF_NAMESPACE "operators.coreos.com/ibm-automation-eventprocessing.$IAF_NAMESPACE"
    unsubscribe "" $IAF_NAMESPACE "operators.coreos.com/ibm-automation-flink.$IAF_NAMESPACE"
    unsubscribe "" $IAF_NAMESPACE "operators.coreos.com/ibm-automation.$IAF_NAMESPACE"
    unsubscribe "" $IAF_NAMESPACE "operators.coreos.com/ibm-common-service-operator.$IAF_NAMESPACE"
                   

    oc delete operandrequest iaf-operator -n $IAF_NAMESPACE --ignore-not-found
    oc delete operandrequest iaf-core-operator -n $IAF_NAMESPACE --ignore-not-found
